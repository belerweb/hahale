<?php
/*
Plugin Name: Video Thumbnails for deTube
Plugin URI: http://sutherlandboswell.com/projects/wordpress-video-thumbnails/
Description: <strong>This is a modified version of "Video Thumbnails" plugin for better integrate it with "deTube" theme.</strong> Automatically retrieve video thumbnails for your posts and display them in your theme. Currently supports YouTube, Vimeo, Blip.tv, Justin.tv, Dailymotion and Metacafe.
...
function get_video_thumbnail( $post_id = null ) {
	...
	// If the thumbnail isn't stored in custom meta, fetch a thumbnail
		...
		/*= Modified by Cloud Stone on 2012/07/10 */
		...
		if ( isset( $matches[1] ) ) {
			...
			$youtube_maxresthumb = 'http://img.youtube.com/vi/' . $matches[1] . '/maxresdefault.jpg';

			// Check to make sure it's an actual thumbnail
			if ( ! function_exists( 'curl_init' ) ) {
				$new_thumbnail = $youtube_thumbnail;
			} else {
				$ch = curl_init( $youtube_maxresthumb );
				...
				curl_close( $ch );
				if ( $retcode == 200 ) {
					 $new_thumbnail = $youtube_maxresthumb;
				} else {
					$new_thumbnail = $youtube_thumbnail;
				}
			}
			...
			if ( isset( $matches[1] ) ) {
				...
				if ( is_wp_error( $vimeo_thumbnail ) ) {
				  return $vimeo_thumbnail;
					...
					/* Do we really need 1280 width?
						if(getimagesize($vimeo_1280thumb) != false)
						$new_thumbnail = $vimeo_1280thumb;	
					*/
					if(getimagesize($vimeo_960thumb) != false)
...
<div class="wrap">
	...
	<div id="message" class="updated"><p><strong>This is a modified version of "Video Thumbnails" plugin for better integrate it with "deTube" theme.</strong></p></div>
