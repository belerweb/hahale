...
//ACCESS_ID
define('OSS_ACCESS_ID', 'a');

//ACCESS_KEY
define('OSS_ACCESS_KEY', 'a');
...
class ALIOSS{
	...
	public function auth($options){
		...
		if ($this->debug_mode){
			$request->debug_mode = false;//$this->debug_mode;
		}
		...
		/*new ResponseCore ( $response_header , $request->get_response_body (), $request->get_response_code () );
		...
		*/
		...
		preg_match_all( "/\<Code\>(.*?)\<\/Code\>/", $data, $returncode ); 
		//print_r($data);
		($returncode[1][0]=="SignatureDoesNotMatch"||$returncode[1][0]=="InvalidAccessKeyId")?$this->signerr=true:$this->signerr=false;
		$this->errcode=$returncode[1][0];
		$this->redirects = 0;
		return $data;
	}
	 ...
	 */
		...
		//为防止在配置时请求数猛增，尝试获取据库中的缓存，失败则从get_bucket_acl接口获取
				...
				if((!$breads[$value])||(time()-$breads_updates>86400)){ //缓存时间，1天
					$eflag=1;
					$a=$this->get_bucket_acl($value,$options);
